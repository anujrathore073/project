...
if "expense_tracker" not in session_state:
    ...
    session_state.expense_tracker = ExpenseTracker()
...
if selected == "Data Entry":
    ...
    with st.expander("Add  Member"):
        ...
        if st.button("Add Member"):
            try:
                ...
                if not member:
                    expense_tracker.add__member(
                        member_name, earning_status, earnings
                    )
                    st.success("Member added successfully!")
                # Else, update it
                else:
                    expense_tracker.update__member(
    ...
    with st.expander("Add Expenses"):
        ...
        expense_value = st.number_input("Total", min_value=0)
    ...
    if not expense_tracker.members:
        ...
        for member in expense_tracker.members:
            ...
            if _delete_column.button(f"Delete {member.name}"):
                expense_tracker.delete__member(member)
                st.rerun()
