...
class  ExpenseTracker:
    ...
    def add__member(self, name, earning_status=True, earnings=0):
        if not name.strip():
            raise ValueError("Name field cannot be empty")

        member = Member(name, earning_status, earnings)
        self.members.append(member)
    
    def delete__member(self, member):
        self.members.remove(member)

    def update__member(self, member, earning_status=True, earnings=0):
        if member:
